...
class RepresentativeListAdapter(): ListAdapter<Representative, RepresentativeListAdapter.RepresentativeViewHolder>(RepresentativeDiffCallback) {
    ...
    class RepresentativeViewHolder(val binding: ItemRepresentativeBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        private fun showSocialLinks(channels: List<Channel>) {
            ...
            if (!facebookUrl.isNullOrBlank()) {
                enableLink(binding.ivFacebookLink, facebookUrl)
            }

            val twitterUrl = getTwitterUrl(channels)
            if (!twitterUrl.isNullOrBlank()) {
                enableLink(binding.ivTwitterLink, twitterUrl)
            }
        }

        private fun showWWWLinks(urls: List<String>) {
            enableLink(binding.ivWwwLink, urls.first())
        }
    ...
    companion object RepresentativeDiffCallback : DiffUtil.ItemCallback<Representative>() {
        override fun areItemsTheSame(oldItem: Representative, newItem: Representative) = oldItem.office.name === newItem.office.name
